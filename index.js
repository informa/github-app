...
module.exports = (app) => {
  ...
  app.on("schedule.repository", async (context) => {
    ...
    // 4. create JWT and clone repo
    const github = await app.auth(); // Not passing an id returns a JWT-authenticated client
    const token = await github.apps.createInstallationToken({
      installation_id: context.payload.installation.id,
    });

    Git.Clone(
      `https://x-access-token:${token.data.token}@github.com/${login}/${repoName}.git`,
      `repos/${repoName}`
    ).then(function (repository) {
      // Work with the repository object here.
    });

    // OR build file names using https://developer.github.com/v3/git/trees/ and scan them.
